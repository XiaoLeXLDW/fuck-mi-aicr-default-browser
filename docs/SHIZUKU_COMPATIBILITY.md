...
**当前 v0.3.1 已实现官方 Shizuku API 与 Stellar 原生 API 两条后端；当前版本的双后端实机验收尚未完成。** 本页描述当前源码，历史结论放在文末，避免把旧版的 Stellar-only 限制套到新版。
...
两条路径共享 `RedirectorUserService`，共用协议 `200` 与服务代 `30002`，但每个会话保存创建它的后端。管理器版本、Binder 可用性与服务 UID/PID 均由运行状态提供，不应从按钮名称推断真实连接。
...
| 范围 | 当前结论 |
| --- | --- |
| v0.3.1 源码、构建、单测、Lint、双 Provider 打包 | 已有静态检查记录，详见[构建证据](BUILD_EVIDENCE.md) |
| v0.3.1 官方 Shizuku 与 Stellar | 授权、真实跳转、停止、重开、故障恢复均需实机验收 |
| v0.3.1 浏览器弹窗 | 真机视觉、大字体、横屏/折叠、TalkBack 待验 |
| v0.2.0 Stellar | 历史记录包含观察、Firefox 接管、停止与重开；只证明该历史版本 |
...
v0.2.1 及此前的特权接入只有 Stellar 原生链路，没有官方 Shizuku API 依赖与 `.shizuku` Provider；当时不能称为官方 Shizuku 兼容。v0.3.0 增加了双适配器与独立生命周期，v0.3.1 更新了浏览器选择菜单。
