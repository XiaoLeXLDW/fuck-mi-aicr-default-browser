...
**v0.3.4 实现了官方 Shizuku API 与 Stellar 原生 API 两条后端。**
本版仍待手机验收。
...
| --- | --- | --- | --- | --- |
| v0.3.4 | 11 | 200 | 30005 | Pre-release；本版手机验收待完成 |

页面“兼容服务（原生 API）”对应 **Stellar 原生路径**。
...
v0.3.4 的 Stellar 原生连接记录也固定保存来源管理器和其 Binder，不将旧 token 发给
...
| 范围 | 当前结论 |
| --- | --- |
| v0.3.4 构建与发布信息 | 校验与 CI 信息见[版本说明](releases/v0.3.4.md) |
| 官方 Shizuku 与 Stellar 原生路径 | 本版待手机验收；双管理器、ADB 冷启动及异常恢复也无完整实机证据 |
| 浏览器弹窗 | 真实候选过滤、大字体、横屏/折叠、TalkBack 仍待专项验收 |
