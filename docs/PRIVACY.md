...
本说明面向 **v0.3.4**，按现有 `RedirectDispatcher`、`AmProcessLauncher` 和本地配置实现说明数据流。“岛外打开”在设备上读取浏览器启动事件，将符合规则的网址交给所选浏览器。当前源码没有账号系统、分析 SDK 或上传实现；应用 Manifest 不声明 `INTERNET` 权限。
