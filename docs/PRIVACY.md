...
本说明适用于 **v0.3.5**。“岛外打开”在设备上读取浏览器启动事件，将符合规则的网址交给所选浏览器。应用没有账号系统、分析 SDK 或上传功能；Manifest 不声明 `INTERNET` 权限。
...
展示域名，但域名本身也可能敏感；权限、连接和 Controller 等其他错误消息也可能包含敏感信息。
...
页面“Stellar（原生 API）”指 Stellar；它与官方 Shizuku 的授权分别处理。
...
`scripts/verify-stop.ps1` 会安装/启动应用并操作界面；指定 `-SkipInstall` 时跳过安装。
UI 自动化会在设备 `/sdcard/codex-redirector-<随机 GUID>.xml` 写入页面转储，每次运行使用独立文件名。
脚本退出时仅尝试删除本次转储；ADB 断线、脚本被强制终止或删除失败时可能残留。
它不是只读诊断脚本，转储和终端输出需要脱敏后再分享；处理残留时只删除确认属于本次运行的文件。

## 停用与清除

先在应用内点“停用并退出服务”，等待“停止完成”及 Binder 死亡确认。随后如需清除配置，可通过 Android 的应用存储设置清除数据或卸载；这会重置已保存选择。清除应用数据不会清除浏览器历史、系统日志或手动导出的诊断文件。

使用步骤见[README](../README.md)，故障处理见[FAQ](FAQ.md#停止未完成怎么恢复)。
