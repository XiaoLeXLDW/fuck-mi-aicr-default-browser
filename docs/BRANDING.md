...
浏览器徽章包含地球与地址栏，位于角色右下角并在圆形裁切范围内保留完整轮廓。
启动器延展底色为 `#141A20`，与原画边缘衔接。
图标与仓库展示保持同一角色；应用功能、版本状态和兼容性仍以 [README](../README.md) 为准。

## 家族来源

当前原画参考以下仓库的图标，来源固定到对应源码版本：
...
以无损 WebP 保存，作为仓库展示图和 Android 资源的统一输入。

原画采用不透明石墨背景；导出器为仓库图标添加透明外边距，为 Android 自适应图标保留系统裁切空间。
App 浅色界面的主按钮使用深青色 `#086F7E` 搭配白色文字。
...
单色矢量保留小尺寸下的功能轮廓。启动器是否显示主题图标取决于系统和用户设置；
HyperOS 桌面、App 实际显示与主题动画仍待本版实机确认。

## 许可与署名

导出脚本与本项目矢量图代码采用根目录 [MIT 许可证](../LICENSE)。
当前仓库未对生成原画及其派生 PNG 单列图像许可证；已知参考来源见上表。
参考项目素材保留各自权利，本说明不重新授权。完整来源登记见[第三方来源声明](../THIRD_PARTY_NOTICES.md)。
...
当前 APK 的打包、签名和校验信息见[构建证据](BUILD_EVIDENCE.md)与[版本说明](releases/v0.3.4.md)。
